...
@TeleOp
public class DriverControlsDouble extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            ...
            if (driverOp.getTrigger(GamepadKeys.Trigger.LEFT_TRIGGER) > 0.5) {
                SPEEDCONTROL = .4;
                TURNCONTROL = .4;
            } else if (driverOp.getTrigger(GamepadKeys.Trigger.RIGHT_TRIGGER) > 0.5) {
                TURNCONTROL = 0.4;
                SPEEDCONTROL = 1;
            } else {
                SPEEDCONTROL = 1;
            ...
            if(gamepad2.touchpad) {
                if(gamepad2.touchpad_finger_1_x < 0 && gamepad2.touchpad_finger_1_y < 0){
                    changeHorizontalPosition(robot.horizontal1.getCurrentPosition() - 10, robot.horizontal2.getCurrentPosition() - 10, 0.3);
                }else if(gamepad2.touchpad_finger_1_x > 0 && gamepad2.touchpad_finger_1_y < 0){
                    changeHorizontalPosition(robot.horizontal1.getCurrentPosition() + 10, robot.horizontal2.getCurrentPosition() + 10, 0.3);
                }else if(gamepad2.touchpad_finger_1_x < 0 && gamepad2.touchpad_finger_1_y > 0){
            ...
            // Wrist Adjustment
            if (Math.abs(gamepad2.left_trigger) > 0 && Math.abs(gamepad2.right_trigger) > 0) {
                wristPos = 0.158;
                setWrist();
            } else if (Math.abs(gamepad2.left_trigger) > 0 ) {
                wristPos -= Math.abs(gamepad2.left_trigger) / 100;
                if (wristPos < 0.158) {
                    wristPos = 0.158;
                }
                setWrist();
            } else if (Math.abs(gamepad2.right_trigger) > 0 ) {
                wristPos += Math.abs(gamepad2.right_trigger) / 100;
                if (wristPos > 0.503) {
                    wristPos = 0.503;
                }
                setWrist();
            ...
            telemetry.addData("| Touchpad Fing 1: ", Boolean.toString(gamepad2.touchpad_finger_1));
            telemetry.addData("| Touchpad Fing 2: ", Boolean.toString(gamepad2.touchpad_finger_2));
            telemetry.addData("| Touchpad Fing 1x: ", Float.toString(gamepad2.touchpad_finger_1_x));
            telemetry.addData("| Touchpad Fing 1y: ", Float.toString(gamepad2.touchpad_finger_1_y));
            telemetry.addData("| Touchpad Fing 1 Press: ", Boolean.toString(gamepad2.touchpad));
